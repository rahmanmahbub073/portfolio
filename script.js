$(document).ready(function(){
    ...
    // Check if the navigation menu is overflowing
    function checkNavOverflow() {
        const navContainer = document.querySelector('.menu-wrapper');
        const menu = document.querySelector('.navbar .menu');
        
        if (navContainer && menu) {
            // If menu width is greater than container width in non-mobile view
            if (window.innerWidth > 991 && menu.scrollWidth > navContainer.clientWidth) {
                menu.style.justifyContent = 'flex-start';
                menu.style.overflowX = 'auto';
                menu.style.paddingBottom = '5px'; // Add space for potential scrollbar
    ...
    checkNavOverflow();
    $(window).resize(checkNavOverflow);
    ...
    $('.filter-btn').click(function(){
        ...
        if(value == 'all'){
            ...
            $('.project-item').not('[data-category*="' + value + '"]').hide('1000');
            $('.project-item').filter('[data-category*="' + value + '"]').show('1000');
        }
    });
    ...
    $('.skill-filter-btn').click(function(){
        ...
        if(value == 'all'){
            $('.skill-card').show('1000');
        }
        else{
            $('.skill-card').not('[data-category="' + value + '"]').hide('1000');
            $('.skill-card').filter('[data-category="' + value + '"]').show('1000');
        }
    });
    ...
    $('.award-filter-btn').click(function(){
        ...
        if(value == 'all'){
            $('.award-item').show('1000');
        }
        else{
            $('.award-item').not('[data-category="' + value + '"]').hide('1000');
            $('.award-item').filter('[data-category="' + value + '"]').show('1000');
        }
    });
    ...
    $('.publication-filter-btn').click(function(){
        ...
        if(value == 'all'){
            $('.publication-item').show('1000');
        }
        else{
            $('.publication-item').not('[data-category="' + value + '"]').hide('1000');
            $('.publication-item').filter('[data-category="' + value + '"]').show('1000');
        }
    });
    ...
    if($('#navigation-placeholder').length) {
        $("#navigation-placeholder").load("components/navigation.html", function(){
            // Re-run navigation checks after loading
            checkNavOverflow();
            
            // Set active class based on current page
            const currentLocation = window.location.pathname;
            $('.navbar .menu li a').each(function(){
                const linkPath = $(this).attr('href').split('#')[0];
...
// Ensure proper browser display for navigation
document.addEventListener('DOMContentLoaded', function() {
    // Force browser to recalculate menu layout after full load
    setTimeout(function() {
        const menu = document.querySelector('.navbar .menu');
        if (menu) {
